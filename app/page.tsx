...
function InventoryForm({ initialData = {}, onSave, sourceOptions = ['社團', '韓拍', 'Mercari', '推特', '蝦皮'] , uniqueSources, onRenameSource, onDeleteSource}) {
    ...
    // 🌟 確保表單擁有一個穩定的 ID，若 initialData 沒有 (新增模式)，則預先產生一個，避免每打一個字就存成新的一筆
    const [stableId] = useState(() => initialData.id || `inv_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`);
    const dataIdRef = useRef(stableId);
    const timerRef = useRef(null);
