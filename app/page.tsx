    ...
    const syncToParent = (nextForm) => {
        ...
        timerRef.current = setTimeout(() => {
            onSave({
                ...
                sellDate: (nextForm.sellDate && nextForm.sellDate !== '') ? nextForm.sellDate : null
            }, (newId) => {
        ...
        try {
            ...
            await Promise.all(imgElements.map(async img => {
                ...
                try {
                    ...
                    try {
                        const pRes = await fetch(`/api/proxy-image?url=${encodedUrl}`, { cache: 'no-cache' });
                        if (pRes.ok) {
    ...
    const handlePocaCrawl = async () => {
        ...
        try {
            ...
            (cards || []).forEach(c => {
                const pocaId = c.poco_id || c.pocoId || c.poco_jd || c.pocaCard || c.PocaCard || c.poca_id;
                if (pocaId) {
                    matchedPocaIds.add(String(pocaId));
                }
            });
            ...
            const CHUNK_SIZE = 10; // D1 has a parameter limit (around 100). Reducing chunk size to 10 (50 params) to safely avoid this limit.
            ...
            // 🌟 致命閃退修正：延遲 alert 的顯示，避免在 React 大量渲染 DOM (幾千筆卡片) 時，卡住 Safari 的 UI 執行緒造成 AppKit 崩潰
            setTimeout(() => {
...
// 🌟 自動統整所有庫存與盤收的「來源」
  ...
  const handleSaveData = async (data, type, isEdit = false) => {
      ...
      if (type === 'batch' && isEdit) {
          const cardPayload = {};
          // 🌟 致命錯誤修正：只在有設定值 (truthy) 時才向下覆寫卡片，避免批次留空時把卡片原本的分類洗成 null
          if (payload.seriesId) cardPayload.series_id = payload.seriesId;
          if (payload.channel) cardPayload.channel = payload.channel;
          if (payload.type) cardPayload.type = payload.type;
          ...
          if (Object.keys(cardPayload).length > 0) {
              const updatedCards = cards.map(c => {
                  if (c.batchId === payload.id) {
                      return { 
                          ...c, 
                          seriesId: payload.seriesId || c.seriesId, 
                          channel: payload.channel || c.channel, 
  ...
  const handleSaveBulkRecord = async (data) => {
      ...
      const newBulkInvItems = (dataToSave.items || []).filter(item => !item.isMisc).map((item, idx) => {
          ...
          return {
              ...
              sellDate: (item.sellDate && item.sellDate !== '') ? item.sellDate : ((existing?.sellDate && existing.sellDate !== '') ? existing.sellDate : null), // 🌟 確保空日期轉為 null，避免 SQLite 寫入失敗
