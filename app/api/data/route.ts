...
export async function GET(request: Request) {
  try {
    ...
    if (!env) {
      return NextResponse.json({ error: '環境變數 env 尚未載入。如果您在本地端測試，請確保設定了 setupDevPlatform。' }, { status: 400 });
    }
    if (!env.DB) {
      return NextResponse.json({ error: '找不到 D1 資料庫綁定！請確認您已在 Cloudflare Pages 後台 [Settings] > [Functions] 綁定了名為 "DB" 的 D1 資料庫。' }, { status: 400 });
    }
